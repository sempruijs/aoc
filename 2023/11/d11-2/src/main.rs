...
impl Galaxy {
    ...
    fn add_lightyear_row(&self, transpose: bool) -> Self {
        let numbers: HashSet<i64> = match transpose {
        ...
        let total_numbers: HashSet<i64> = HashSet::from_iter(*min_n..=*max_n);
    ...
    fn sum_distances(&self) -> u64 {
        ...
        pairs.iter().map(|p| distance(&p.0, &p.1)).sum::<u64>()
    }
...
fn distance(p1: &Point, p2: &Point) -> u64 {
    ...
    let x_div: i64 = match p1.x >= p2.x {
        true => p1.x - p2.x,
        false => p2.x - p1.x,
    };

    (y_div + x_div).abs() as u64
}
